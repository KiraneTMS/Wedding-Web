function initTheme(data) {
  ...
  // 2. Gallery images (hero bg + profile hex-frames)
  ...
  if (storyList && data.story) {
    storyList.innerHTML = data.story.map(item => `
      <div class="flex gap-4">
        <span class="text-[#c9a66b] text-xl leading-none mt-1">🌿</span>
        <div>
          ...
          <p class="text-[#f5f1e8]/60 text-sm">${item.desc}</p>
        </div>
      </div>
    `).join('');
  }
  ...
  // 6. Gallery — small hexagon frames
  const galleryGrid = document.getElementById('gallery-grid');
  if (galleryGrid && data.gallery) {
    galleryGrid.innerHTML = data.gallery.map(url => `
      <div class="hex-frame" style="width: 140px; height: 155px;">
  ...
  // 9. Drifting gold-fleck / leaf particles
...
function spawnGardenParticles() {
  ...
  // Kilau emas & daun tropis melayang menyamping perlahan naik ke atas
  const symbols = ['🌿', '✦', '🍃'];
